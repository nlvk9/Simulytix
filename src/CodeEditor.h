...
struct Variable
{
    ...
    std::string type;
    std::string value;
    int line = -1; // Line number where it's declared
};
...
public:
    ...
    const std::map<std::string, Variable> &getVariables() const { return m_variables; }
...
private:
    ...
    // Helper functions for enhanced syntax highlighting
    enum TokenType
    {
        TT_UNKNOWN,
        TT_KEYWORD,
    ...
    TokenType getTokenType(const std::string &word, bool followedByParen);
    void parseVariablesFromCode();
