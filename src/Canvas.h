...
public:
    ...
    bool isComponentSelected(int componentId) const
    {
        return m_selected >= 0 && m_selected < (int)m_components.size() &&
               m_components[m_selected].id == componentId;
    }

    // Grid snapping
    void setGridSnap(bool enabled) { m_gridSnap = enabled; }
    bool gridSnap() const { return m_gridSnap; }
    float gridSize() const { return 20.0f; }
...
public:
    int componentIndexById(int compId) const;
...
private:
    ...
    bool m_gridSnap = true; // Enable grid snapping by default
};
